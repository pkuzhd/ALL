...
struct CardCombo
{
	...
	*/
	...
	template <typename CARD_ITERATOR>
	CardCombo myMain(CARD_ITERATOR begin, CARD_ITERATOR end) const
	{

		CardCombo myMainCard;
		// 然后先看一下是不是火箭，是的话就过
		if (comboType == CardComboType::ROCKET)
			return CardCombo();
		...
		{
			counts[card2level(c)]++;
			Cards[card2level(c)].push_back(c);
		}
		...
		if (comboType == CardComboType::PASS) // 如果不需要大过谁，只需要随便出
		{
			...
			// if (false)
			{
				for (int i = 0; i <= 10; ++i)
				{
					if (Cards[i].size() == 3)
					{
						myMainCard.cards = Cards[i];
						int k = 1;
						while (Cards[i + k].size() == 3)
						{
							for (int m = 0; m < 3; ++m)
								myMainCard.cards.push_back(Cards[i + k][m]);
							++k;
						}
						break;
					}
				}
				int num = myMainCard.cards.size() / 3;
				for (int m = 0; m < num; ++m)
				{
					for (int i = 0; i <= 10; ++i)
					{
						if (Cards[i].size() == 1)
						{
							myMainCard.cards.push_back(Cards[i][0]);
							if (myMainCard.cards.size() == num * 4)
								return myMainCard;
						}
					}
			...
			myMainCard.cards.clear();
			// 单顺
			{
				for (int i = 0; i <= 7; ++i)
				{
					if (!(Cards[i].size() >= 1 && Cards[i].size() <= 3))
						continue;
					int num = 1;
					for (int j = i + 1; ; ++j)
					{
						if (j == 12)
							break;
						if (Cards[j].size() >= 1 && Cards[j].size() <= 3)
						{
							++num;
						}
						else
					...
					if (num >= 5)
					{
						...
						{
							myMainCard.cards.push_back(Cards[j][0]);
						}
						return myMainCard;
					}
				}
			...
			// 双顺
			{
				for (int i = 0; i <= 7; ++i)
				{
					...
					if (num >= 3)
					{
						...
						{
							for (int k = 0; k < 2; ++k)
								myMainCard.cards.push_back(Cards[j][k]);
						}
						return myMainCard;
					}
				}
...
int main()
{
	...
	CardCombo myAction = lastValidCombo.myMain(myCards.begin(), myCards.end());
